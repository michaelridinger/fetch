...
.acc .hitrow_callnumber {
	font-size: 2em;
}
.acc .details_holdings {
font-size: 1.2em;
}
.acc .details_holdings td {
...
.acc input, .acc_input-group, .acc .hitrow_summary, .acc .hitrow_pub, .acc .hitrow_author, .acc .hitrow_title {
...
<?
	if ($_SESSION['fetchbgseries'] == "safari") {
		$bgimg = "https://fetch.infohio.org/images/series/".$_SESSION['fetchbgseries']."/".strtolower($_SESSION['fetchbgseries']."_".sprintf("%02d",date("d"))).".jpg";
	} elseif (strpos($_SESSION['fetchbg'],".mp4")) {
		$bgimg="";
	} elseif (strpos($_SESSION['fetchbg'],"nobg")) {
		$bgimg = "";
	} elseif ($_SESSION['fetchbg'] == "calendar") {
		$bgimg = "https://fetch.infohio.org/images/background/fetch_light.png";
	} elseif (isset($_SESSION['fetchbgcustom'])) {
		$bgimg = $_SESSION['fetchbgcustom'];
	} elseif (strlen($_SESSION['fetchbg']) > 3) {
		$bgimg = $_SESSION['fetchbg'];
	} else {
		$_SESSION['fetchbg']="https://fetch.infohio.org/images/background/dark.png";
		$bgimg = "https://fetch.infohio.org/images/background/".$_SESSION['fetchbg'];
	}
?>
body {
	height:100%;
...
.acc {
	background-image: url("/images/background/fetch_allgrades.png");
}
#tryisearchbox {
	font-size:.7em;
	color: #FF0000;
...
.cat-mainmenu {
	...
	border: 1px solid <? echo $_SESSION['color1']; ?>;
}
.acc .cat-mainmenu {
	margin: auto;
	color: #FFF;
