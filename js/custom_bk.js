$(document).ready(function(){
	...
	$(document).on("click",".placehold",function() {
		...
		var sw = screen.width;
		var lft = sw/2-300;
		window.open(url,"Place Hold","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,copyhistory=no,width=400,height=600, top=100,left="+lft);
	});	
	$(document).on("click","#showisearchresultsbutton",function() {
		var url = $("#isearchresultsurl").val();
		var sw = screen.width;
		var lft = sw/2-300;
		window.open(url,"ISearch Results","toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,copyhistory=no,width=400,height=600, top=100,left="+lft);
	});	
	...
	if ($("#searchform").length) {
		if ($("#startnewsearch").val() != "Y" && ($("#q").val() > "" || $("#rptype").val() > "")) {	
			$("#warning").hide();
			$("#warningtext").html("");
			$("#loader").show();
			$(".menu_tryagain").show();
		...
		/*
		var gurl = "https://fetch.infohio.org/isearch.php?"+$("#moreisearchresults").val();
		$.get(gurl,function(data) {
			var hits = data*1;
			if (hits > 0) {
				var iurl = $("#isearchurl").val()+"?qu="+$("#q").val();
				$("#tryisearchbox").html("<br>(<a target='_blank' style='color:#ff0000' href='"+iurl+"' data-uk-tooltip title='See "+hits+" results'>View "+hits+" ISearch Results</a>)");
				$("#tryisearchbox").show();
			}
		});
		*/
	...
	function getcovers() {
		$(".catcover").each(function() {
			...
			$(this).attr("src","https://syndetics.com/index.aspx?isbn="+isbn+"/MC.GIF&client=419-222-7417&type=unbound&upc=&oclc=&issn=cover&");
		});
		$(".catcover2").each(function() {
			var isbn = $(this).attr("id").replace("image_","");
			$(this).attr("src","https://syndetics.com/index.aspx?isbn="+isbn+"/LC.GIF&client=419-222-7417&type=unbound&upc=&oclc=&issn=cover&");
		});
	}
